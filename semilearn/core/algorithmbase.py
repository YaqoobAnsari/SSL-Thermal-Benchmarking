...
from semilearn.core.hooks import Hook, get_priority, CheckpointHook, TimerHook, LoggingHook, DistSamplerSeedHook, ParamUpdateHook, EvaluationHook, EMAHook, WANDBHook, AimHook, MetricsLoggerHook
...
class AlgorithmBase:
    ...
    def set_hooks(self):
        ...
        self.register_hook(MetricsLoggerHook(), None, "LOWEST")
    ...
    def process_batch(self, input_args=None, **kwargs):
        ...
        for arg, var in kwargs.items():
            ...
            # Convert labels to Long type for compatibility
            if 'y_' in arg and not isinstance(var, dict):
                var = var.long()

            # send var to cuda if available
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        # Only calculate top-5 for multi-class (>= 5 classes)
        if self.num_classes >= 5:
            top5 = top_k_accuracy_score(y_true, y_probs, k=5)
        else:
            top5 = top1  # For binary/few-class problems, top-5 equals top-1
        ...
        # Save confusion matrix and logits for test set
        if eval_dest == 'test':
            save_dir = os.path.join(self.args.save_dir, self.args.save_name)
            os.makedirs(save_dir, exist_ok=True)
            
            # Save confusion matrix
            cf_mat_path = os.path.join(save_dir, 'confusion_matrix.npy')
            np.save(cf_mat_path, cf_mat)
            self.print_fn(f'Confusion matrix saved to: {cf_mat_path}')
            
            # Save logits
            logits_path = os.path.join(save_dir, 'test_logits.npy')
            np.save(logits_path, y_logits)
            self.print_fn(f'Test logits saved to: {logits_path}')
            
            # Save predictions and labels for further analysis
            predictions_path = os.path.join(save_dir, 'test_predictions.npz')
            np.savez(predictions_path, y_true=y_true, y_pred=y_pred, y_probs=np.array(y_probs))
            self.print_fn(f'Test predictions saved to: {predictions_path}')
